...
#            print dist
    ...
    def graphGrid(self):
        ...
        for i,v in enumerate(self.elements):
            ...
            lab = '({:.6},{:.6})-({:.6},{:.6})'.format(source[0],source[1],target[0],target[1])
            plt.scatter(self.data.variables['lonc'][v],self.data.variables['latc'][v],s=80,label=lab,c=plt.cm.Set1(i/zz))
